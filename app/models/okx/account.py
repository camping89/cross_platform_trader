...
class OKXAccount(BaseModel):
    ...
    total_eq: str = Field(..., description="Total equity in USD")
    adj_eq: Optional[str] = Field(None, description="Adjusted equity in USD")
    iso_eq: str = Field(..., description="Isolated margin equity in USD")
    ord_froz: str = Field(..., description="Cross margin frozen for pending orders in USD")
    ...
    notional_usd: str = Field(..., description="Notional value in USD")
    mgnRatio: str = Field(..., description="Margin ratio")
