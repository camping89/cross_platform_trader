...
from app.models.discord.message import DiscordFetchRequest
        ...
        try:
            ...
            if discord_data:
                ...
                if saved:
                    ...
                    total_count = await self.discord_service.get_message_count_by_user(discord_data.target_user_id)
                    
                    self.logger.info(
                        f"Successfully fetched and saved {discord_data.exported_count} new messages "
                        f"from {discord_data.username}. Total messages in DB: {total_count}"
                    )
                else:
