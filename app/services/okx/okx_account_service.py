...
from ...models.okx.account import (
    OKXAccount, OKXBalance, OKXLeverage, OKXMaxSize,
...
class OKXAccountService:
    ...
    async def get_account_info(self) -> Optional[OKXAccount]:
        ...
        Returns:
            Optional[OKXAccount]: Account information if successful
        """
        if not await self.base_service.ensure_connected():
            return None

        try:
            result = self.base_service.account_api.get_account()
    ...
    async def get_balances(self, ccy: str = None) -> List[OKXBalance]:
        ...
        try:
            ...
            result = self.base_service.account_api.get_account_balance(**params)
