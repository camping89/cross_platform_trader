...
from app.trading_app.models.mt5.signal import TradingSignal
from .mt5_base_service import MT5BaseService
from app.trading_app.config import trading_settings
from app.shared.utils.exceptions import DatabaseConnectionError, SignalNotFoundError
...
class MT5SignalService:
    ...
    def __init__(self, base_service: MT5BaseService):
        ...
        try:
            if not mt5.initialize():
                ...
                raise DatabaseConnectionError("Could not initialize MT5")
            self.client: AsyncIOMotorClient = motor.motor_asyncio.AsyncIOMotorClient(
                trading_settings.MONGODB_URL
            )
            self.db: AsyncIOMotorDatabase = self.client[trading_settings.MONGODB_DB]
    ...
    async def get_distinct_symbols(self) -> List[dict]:
        """
        Get list of distinct symbols with their full names from signals collection.
        
        Returns:
            List[dict]: List of dictionaries containing symbol code and full name
            Example: [{"code": "BTCUSD.p", "name": "Bitcoin - BTCUSD.p"}, ...]
            
        Raises:
            DatabaseConnectionError: If database operation fails
        """
        try:
            symbol_codes = await self.signals.distinct("symbol")
            symbols = []
            
            for code in symbol_codes:
                symbol_info = mt5.symbol_info(code)
                if symbol_info is not None:
                    description = getattr(symbol_info, 'description', code)
                    name = f"{description} - {code}"
                else:
                    name = code
            ...
            logger.info(f"Retrieved {len(symbols)} distinct symbols with names")
            return symbols
        except Exception as e:
